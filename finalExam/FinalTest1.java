...
public class FinalTest1 {
	...
	public static void main(String[] args) {
		...
		boolean working = camelHumps(10, 0) == 15;
...
//*******************************************Recursion***************************************************
	...
	// TODO fill in this method: DONE & COMPLETE
	...
	static int camelHumps(int camels, int camelHumps) {
	    // if camels is 0 before or after recursion, return
	    if(camels == 0){
            System.out.println();
	        return camelHumps;
        }

	    // if-else to output if the camel in the line has 1 or 2 humps
	    if(camels%2 == 0){
            System.out.printf("\nCamel #%d has %d humps.", camels, 2);
            camelHumps+=2;
        } else {
            System.out.printf("\nCamel #%d has %d hump.", camels, 1);
            camelHumps++;
        }

	    // recursive call if camels is greater than 0
        return camelHumps(camels-1, camelHumps);
    }

//*******************************************Graphs***************************************************

	// TODO fill in these methods: DONE & COMPLETE
	static Set<String> depthFirstTraversal(GraphHelper graph, String root) {
	    //creating new LinkedHashSets dftSet and visited
        LinkedHashSet<String> dftSet = new LinkedHashSet<>();
        LinkedHashSet<String> visited = new LinkedHashSet<>();
        return dfs(graph, root, dftSet, visited); //returning a String set (dftSet from dfs())
    }
    ...
    static Set<String> dfs(GraphHelper graph, String root, LinkedHashSet<String> dftSet, LinkedHashSet<String> visited) {
        ...
        for (Vertex v: graph.getAdjVertices(root)) {
            if(!visited.contains(v.label)){
                ...
                dfs(graph, v.label, dftSet, visited);
            }
        } //end for-each
...
class BST<E extends Comparable<E>> {
	...
	public boolean contains(E e) {
        // while the root is not null (if null, return false)
        while (root != null) {
            if (e.compareTo(root.element) < 0) {
                root = root.left; // search subtree to the left
            }
            else if (e.compareTo(root.element) > 0) {
                root = root.right; // search subtree to the right
            }
            else {
                return true;
            }
	...
	static void postorder(TreeNode<?> root) {
        if (root == null) return; //if null, no need to put into postorder

        //putting into postorder by left then right, then printing the elements
        postorder(root.left);
...
class MyMaxHeap<T extends Comparable<T>> {
	...
	public void insert(T item) {
        ...
        while (currentIndex > 0) {
            ...
            // setting the current object as the previous parent
...
class Tree24<E extends Comparable<E>> {
  ...
  private void postorder(Tree24Node<E> root) {
      if (isEmpty()) return; //if null, no need to put into postorder

      //for() to put into postorder by visiting each child
      for (int i = 0; i < root.child.size(); i++) {
          postorder(root.child.get(i));
      }
      //for() to print the elements in postorder
      for (int i = 0; i < root.elements.size(); i++) {
          System.out.print(root.elements.get(i) + " ");
      }
  }
